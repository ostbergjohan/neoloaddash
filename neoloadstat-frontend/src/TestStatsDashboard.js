...
const TestStatsDashboard = () => {
    ...
    const calculateOverallStats = () => {
        ...
        data.workspaces
            .filter(ws => !excludedWorkspaces.includes(ws.workspaceId))
            .forEach((ws) => {
    ...
    // Workspace status categorization
    const workspaceData = data.workspaces
        .filter((ws) => ws.tests.length > 0 && !excludedWorkspaces.includes(ws.workspaceId))
        .map((ws) => {
    ...
    const getTestData = () => {
        let filteredWorkspaces = data.workspaces.filter(ws => !excludedWorkspaces.includes(ws.workspaceId));
            ...
            return filteredWorkspaces
                .flatMap((ws) =>
                    ws.tests.map((test) => ({
                        ...
                        displayName: `${test.scenario.substring(0, 20)}${test.scenario.length > 20 ? "..." : ""}`,
                    }))
                )
        ...
        const workspace = filteredWorkspaces.find((ws) => ws.workspaceId === selectedWorkspace);
        return workspace
    ...
    const exportCSV = () => {
        ...
        const rows = workspaceData.map((w) => [
            `"${w.name}"`,
            w.total,
            w.passed,
            w.failed,
    ...
    const overallStatus = parseFloat(passRate) >= 90 ? 'excellent' :
        parseFloat(passRate) >= 70 ? 'good' :
            parseFloat(passRate) >= 50 ? 'attention' : 'critical';
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                        <div>
                            ...
                            <p style={{ color: '#64748b', fontSize: '0.875rem' }}>
                                Comprehensive test performance analysis and reporting
                            </p>
                        </div>
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            >
                                <Download style={{ width: '16px', height: '16px', display: 'inline', marginRight: '4px' }} />
                                Export CSV
                            </button>
                        </div>
                    </div>
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '32px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px', marginBottom: '32px' }}>
                    <div className="hover-shadow" style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', border: '1px solid #e2e8f0', padding: '24px', transition: 'all 0.3s' }}>
                        ...
                        <p style={{ fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>
                            Total Test Runs
                        </p>
                    ...
                    <div className="hover-shadow" style={{
                        backgroundColor: getStatusBadge(overallStatus).bg,
                        borderRadius: '12px',
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                            <div style={{ padding: '12px', backgroundColor: 'rgba(255,255,255,0.5)', borderRadius: '8px' }}>
                                {parseFloat(passRate) >= 70 ? (
                                    <TrendingUp style={{ width: '24px', height: '24px', color: getStatusColor(overallStatus) }} />
                                ) : (
                                    <TrendingDown style={{ width: '24px', height: '24px', color: getStatusColor(overallStatus) }} />
                                )}
                            </div>
                        </div>
                        <p style={{ fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>
                        ...
                        <p style={{ fontSize: '2rem', fontWeight: 'bold', color: getStatusColor(overallStatus) }}>
                            {passRate}%
                        </p>
                        <p style={{ fontSize: '0.75rem', color: getStatusColor(overallStatus), marginTop: '4px', fontWeight: '600' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '24px', marginBottom: '32px' }}>
                    <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', border: '1px solid #e2e8f0', padding: '32px' }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#1e293b', marginBottom: '24px' }}>
                                ...
                                return (
                                    <div key={item.status} style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            ...
                                            <div>
                                                <p style={{ fontSize: '0.875rem', fontWeight: '600', color: badge.color }}>
                                                    {item.label}
                                                </p>
                    ...
                    <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', border: '1px solid #e2e8f0', padding: '32px' }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#1e293b', marginBottom: '16px', textAlign: 'center' }}>
                            Status Distribution
                        </h2>
                ...
                <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '32px' }}>
                    <div style={{ padding: '24px 32px', borderBottom: '1px solid #e2e8f0' }}>
                        ...
                        <p style={{ fontSize: '0.875rem', color: '#64748b' }}>
                            Complete breakdown of all workspace test results
                        </p>
                    </div>
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tr>
                                <th style={{ padding: '16px 32px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>
                                    Workspace
                                </th>
                                <th style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>
                                    Total Runs
                                </th>
                                <th style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>
                                    Passed
                                ...
                                <th style={{ padding: '16px 32px', textAlign: 'center', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>
                                    Status
                                </th>
                            </tr>
                            </thead>
                            ...
                            {workspaceData.map((ws, index) => {
                                ...
                                return (
                                    <tr key={index} className="hover-row" style={{ borderBottom: '1px solid #f1f5f9', transition: 'background-color 0.2s' }}>
                                        <td style={{ padding: '16px 32px', fontSize: '0.875rem', fontWeight: '500', color: '#1e293b' }}>
                                            {ws.name}
                                        </td>
                                        <td style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.875rem', color: '#64748b', fontWeight: '600' }}>
                                            {ws.total.toLocaleString()}
                                        </td>
                                        <td style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: '#059669' }}>
                                            {ws.passed.toLocaleString()}
                                        </td>
                                        <td style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.875rem', fontWeight: '600', color: '#dc2626' }}>
                                            {ws.failed.toLocaleString()}
                                        </td>
                                        <td style={{ padding: '16px 24px', textAlign: 'center', fontSize: '0.875rem', fontWeight: 'bold', color: getStatusColor(ws.status) }}>
                                            {ws.passRate}%
                                        </td>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ flex: '1', minWidth: '250px' }}>
                            ...
                            <select
                                multiple
                                value={pendingExclusions}
                                onChange={handleExcludeWorkspaceChange}
                                style={{
                                    width: '100%',
                                    padding: '8px 12px',
                                    borderRadius: '8px',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', minWidth: '200px' }}>
                            ...
                            {excludedWorkspaces.length > 0 && (
                                <div style={{ padding: '16px', backgroundColor: '#fef9c3', borderRadius: '8px', border: '1px solid #fde047' }}>
                                    <p style={{ fontSize: '0.75rem', color: '#a16207', marginBottom: '4px' }}>
                                        Currently excluded:
                                    </p>
                ...
                <div style={{
                    ...
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginBottom: '12px' }}>
                        Need a real-time display for monitoring?
                    </p>
                    <a
